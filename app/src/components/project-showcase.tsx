...
export function ProjectShowcase() {
  return <div className="project-grid">{projects.map(project => <a data-reveal="project" className="project-feature" href={project.url} key={project.name} target="_blank" rel="noopener noreferrer" aria-label={`Visit ${project.name}${project.name === "Aure Berries" ? " in English" : ""}, opens in a new tab`}>
    <div className="project-image"><img src={assetUrl(`/assets/${project.image}`)} alt={project.alt} loading="lazy" width="1200" height="800"/><span className="project-category">{project.category}</span><span className="project-open" aria-hidden="true">↗</span></div>
    <div className="project-description"><div><h3>{project.name}</h3><p>{project.description}</p></div><span className="project-domain">Explore ↗</span></div>
  </a>)}</div>;
}
...
export function HeroDetails() {
  return <div className="hero-details">
    ...
    <div className="hero-explore"><a className="work-target" href="#work"><span>Explore 04 websites</span><span aria-hidden="true">↗</span></a><span className="scroll-instruction"><span aria-hidden="true">↓</span> Scroll to explore</span></div>

...
export interface ScrollScrubProps {
  ...
  className?: string;
  /** Separate high-resolution cover, dissolved after the first scroll/decoded frame. */
  openingImage?: string;
  ...
  useEffect(() => {
    ...
    const layout = () => {
      ...
      for (const segment of runtime) {
        ...
        // The final chapter ends when its bottom meets the viewport bottom.
        // This lets the last frame paint before the sticky stage releases.
        const finalViewport = segment === runtime.at(-1) ? viewportHeight : 0;
        segment.end = segment.start + Math.max(rect.height - finalViewport, 1);
      }
    ...
    const readScroll = () => {
      ...
      openingProgress = smoothstep((y - 24) / 200);
    ...
    const updateVideos = () => {
      ...
      // Keep the HD cover until a real video frame has painted, including slow connections.
      // Returning to the top always restores the sharp cover.
      if (openingRef.current) {
        const painted = runtime[0]?.layer.dataset.videoPainted === "true";
        openingRef.current.style.opacity = String(painted ? 1 - openingProgress : 1);
      }
    };
  ...
  return (
    ...
    >
      <div className="scroll-scrub__stage">
        ...
        {openingImage ? <img ref={openingRef} className="scroll-scrub__opening" src={openingImage}
          alt="" fetchPriority="high" decoding="async" aria-hidden="true" /> : null}
